...
function DemandeAnneeEncours(int $idEtudiant,string $annee, string $etat):array|null{
  // requete non preparees: requete dont les valeurs sont introduits dans la requete
...
function FindTypeDemande():array|null{
     //  requete parametree
$sql="SELECT DISTINCT `type` FROM `demande`;";
$data=null;
//Classe PDO
//1-Connexion SGBD et Selectionner la BD
  $servername = 'localhost';
...
function addDemande(array $newDemande): void{
   //  requete parametree
     $sql="INSERT INTO `demande` (`type`, `motif`, `date`, `etat`, `id_inscri`) 
      VALUES (:typed,:motif,:dated,:etat,:id_inscri)";
     //Classe PDO
      //1-Connexion SGBD et Selectionner la BD
        $servername = 'localhost';
        $username = 'root';
